...
public class WhaleReducer extends Reducer<IntWritable, DoubleWritable, Whale, DoubleWritable>{
	List<Whale> centers = new LinkedList<Whale>();
	...
	protected void setup(Context context) throws IOException, InterruptedException {	
		...
		centers.add(new Whale(key));
		}
		reader.close();
		Path center1=new Path("hdfs://localhost:9000/usr/seq_file/cen1.seq");
		if(fs.exists(center1))
				fs.delete(center1, true);
			//final SequenceFile.Writer centerWriter1 = SequenceFile.createWriter(fs,conf, center1, Whale.class, IntWritable.class);
		centerWriter = SequenceFile.createWriter(fs,conf, center1, Whale.class, DoubleWritable.class);
		}
	...
	protected void reduce(IntWritable ke, Iterable<DoubleWritable> values, Context context) throws IOException, InterruptedException{
		...
		try {
			centerWriter.append(new Whale(key),new DoubleWritable(key.fitness));
